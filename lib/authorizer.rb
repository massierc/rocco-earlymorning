...
class Authorizer
  ...
  def initialize(username)
    ...
    @credentials_path = File.join(File.dirname(__FILE__), '../credentials/', "#{@username}.yaml")
    ...
    @tg_user = User.find_by_username(@username)
  end
  ...
  def list_activities(cells, project)
    cells.select{|m| m.include?(project)}.map{|x| x[-1]}
  end
